...
class CelebAPipeline:
    """
    Complete pipeline for CelebA experiments including:
    ...
    def _get_image_paths(self, max_images: Optional[int] = None) -> list:
        """Get list of image paths from folder"""
        dataset_config = self.config['dataset']
        image_folder = dataset_config['image_folder']
        pattern = dataset_config.get('image_pattern', '*.png')
    ...
    def step3_run_inference(self) -> str:
        ...
        # Run inference
        output_csv = os.path.join(self.output_dir, self.config['predictions_csv'])
        print(f"Output CSV: {output_csv}")
        
        # Skip if predictions already exist
        if os.path.exists(output_csv):
            print(f"✓ Predictions already exist at {output_csv}")
        ...
        if 'pred_ImgNet21K_name' in df.columns:
            print(f"Prediction distribution (top 10):")
            print(df['pred_ImgNet21K_name'].value_counts().head(10))
        
        if 'pred_name' in df.columns:
            person_count = (df['pred_name'] == 'person').sum()
            total = len(df)
            print(f"Person detection: {person_count}/{total} ({person_count/total*100:.1f}%)")
    ...
    def step3_5_merge_data(self, metadata_path: str, predictions_path: str) -> str:
        ...
        merged_df = pd.merge(metadata_df, predictions_df, on='Filename', how='inner')
        if 'pred' in merged_df.columns:
            merged_df['detection_type'] = merged_df['pred'].map({1: 'TP', 0: 'FN'})
    ...
    def run(self, skip_existing: bool = True):
        ...
        print("=" * 80)
        print(f"RUNNING CELEBA PIPELINE: {self.config['experiment']['name']}")
        print("=" * 80)
...
def main():
    """Main function to run the CelebA pipeline"""
    script_dir = os.path.dirname(os.path.abspath(__file__))
    config_path = os.path.join(script_dir, "configs", "celeba_config.py") 
    
    try:
        # Create and run pipeline
        pipeline = CelebAPipeline(config_path)
        results = pipeline.run(skip_existing=True)
        
        print("\n🎉 CelebA experiment pipeline completed successfully!")
